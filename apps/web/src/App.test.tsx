...
describe('frontend project flow', () => {
  ...
  it('clears an inactive upload when switching to paste text before submit', async () => {
    ...
    const file = new File(['book'], 'source.txt', {
      type: 'text/plain',
    })
  ...
  it('renders persisted image URLs and toggles prompts without generation requests', async () => {
    const detail = {
      id: 'project-1',
      title: 'Saved book',
      createdAt: '2026-01-01T00:00:00.000Z',
      ...
      characters: [
        {
          ...
          portraitUrl:
            '/api/projects/project-1/characters/character-1/portrait',
        },
      ],
      ...
      chapters: [
        {
          ...
          generationError: null,
          illustrationUrl:
            '/api/projects/project-1/chapters/chapter-1/illustration',
        },
      ],
    }
    ...
    expect(
      (await screen.findByAltText('Portrait of Mina')).getAttribute('src'),
    ).toBe(detail.characters[0].portraitUrl)

    expect(
      screen
        .getByAltText('Illustration for The arrival')
        .getAttribute('src'),
    ).toBe(detail.chapters[0].illustrationUrl)
    ...
    const chapterCard = chapterHeading.closest(
      '.chapter-card',
    ) as HTMLElement

    expect(chapterCard.classList.contains('chapter-card--expanded')).toBe(
      false,
    )

    const chapterSummary = chapterCard.querySelector(
      'summary',
    ) as HTMLElement

    expect(chapterSummary.textContent).toContain('View prompt')

    await ui.click(chapterSummary)

    await waitFor(() =>
      expect(
        chapterCard.classList.contains('chapter-card--expanded'),
      ).toBe(true),
    )

    expect(chapterSummary.textContent).toContain('Hide prompt')
    expect(chapterCard.textContent).toContain('Mina arrives')
  ...
  it('prepares an idle book only after an explicit click', async () => {
    ...
    expect(
      await screen.findByRole('button', {
        name: 'Prepare book for generation',
      }),
    ).not.toBeNull()

    expect(
      fetchMock.mock.calls.some(([url]) =>
        String(url).includes('/gemini-book'),
      ),
    ).toBe(false)
    ...
    await waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith(
        '/api/projects/project-1/gemini-book',
        expect.objectContaining({
          method: 'POST',
        }),
      ),
  ...
  it('shows a failed book preparation error and permits only an explicit retry', async () => {
    ...
    expect(
      (await screen.findByText('Gemini book preparation failed.')).textContent,
    ).toContain('failed')
  ...
  it('uses a trimmed manual style and does not automatically run the next step', async () => {
    ...
    await ui.type(
      screen.getByLabelText('Art direction (optional)'),
    ...
    await waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith(
        '/api/projects/project-1/pipeline/STYLE',
        expect.any(Object),
      ),
    )

    const styleCall = fetchMock.mock.calls.find(
      ([url]) => url === '/api/projects/project-1/pipeline/STYLE',
    )

    expect(styleCall?.[1]).toMatchObject({
      body: JSON.stringify({
    ...
    expect(
      fetchMock.mock.calls.some(([url]) =>
        String(url).includes('/pipeline/CHARACTERS'),
      ),
    ).toBe(false)
    ...
    await waitFor(() =>
      expect(
        fetchMock.mock.calls.filter(
          ([url]) => url === '/api/projects/project-1',
        ).length,
      ).toBeGreaterThan(1),
    )
  })
  ...
  it('retries a failed manual STYLE request with the same trimmed manual value', async () => {
    ...
    const retry = fetchMock.mock.calls.filter(
      ([url]) => url === '/api/projects/project-1/pipeline/STYLE',
    )[1][1] as RequestInit
  ...
  it('shows only the failed pipeline step retry and keeps recovery separate while running', async () => {
    ...
    await waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith(
        '/api/projects/project-1/pipeline/PORTRAITS',
        expect.any(Object),
      ),
    )
  })
  ...
  it('shows explicit recovery for running book preparation and surfaces a safe conflict', async () => {
    ...
    const fetchMock = workspaceFetch(detail, (url) =>
      url.endsWith('/gemini-book/recover')
        ? json(
            {
              error: 'There is no stale Gemini book preparation.',
            },
            409,
          )
        : undefined,
    ...
    await screen.findByRole('button', {
      name: 'Recover interrupted preparation',
    })
    ...
    expect((await screen.findByRole('alert')).textContent).toContain(
      'There is no stale Gemini book preparation.',
    )
  })
  ...
  it('uses explicit pipeline recovery while running and redirects on a generation 401', async () => {
    const running = projectDetail({
      pipeline: {
        ...
        runningStep: 'STYLE',
        stepState: 'RUNNING',
        stepStartedAt: '2026-01-01T00:00:00.000Z',
        stepError: null,
      },
    })
    ...
    await ui.click(
      await screen.findByRole('button', {
        name: 'Recover interrupted generation',
      }),
    )

    await waitFor(() =>
      expect(recoveryFetch).toHaveBeenCalledWith(
        '/api/projects/project-1/pipeline/recover',
        expect.any(Object),
      ),
    )
    ...
    const generationFetch = workspaceFetch(projectDetail(), (url) =>
      url.endsWith('/pipeline/STYLE')
        ? json(
            {
              error: 'Authentication required.',
            },
...
function workspaceFetch(
  detail: ReturnType<typeof projectDetail>,
  ...
  >((url: string) => {
    ...
    if (url.includes('/gemini-book') || url.includes('/pipeline/')) {
      return Promise.resolve(
        json({
          status: 'completed',
        }),
